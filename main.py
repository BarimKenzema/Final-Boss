...
print("--- Telegram Scraper v15.0 (Subscription Links + Database Rotation) START ---")
...
def find_subscription_urls(text):
    ...
    url_pattern = r'https?://[^\s<>"{}|\\^`\[\]]+(?:/sub/[^\s<>"{}|\\^`\[\]]+)?'
    urls = re.findall(url_pattern, text, re.IGNORECASE)
    
    # Filter out obvious media files
    excluded_extensions = ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.mp4', '.webm', '.pdf', '.zip', '.rar']
    subscription_urls = []
    ...
    for url in urls:
        ...
        if not any(clean_url.endswith(ext) for ext in excluded_extensions):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if content_length and int(content_length) > MAX_SUBSCRIPTION_SIZE_MB * 1024 * 1024:
                print(f"  ⚠️ Subscription too large: {url} ({int(content_length)/(1024*1024):.1f}MB)")
                return None
            ...
            for chunk in response.iter_content(chunk_size=8192):
                ...
                if len(content) > max_size:
                    print(f"  ⚠️ Subscription exceeded size limit during download: {url}")
                    return None
        ...
        print(f"  ⏱️ Timeout fetching subscription: {url}")
    except Exception as e:
...
async def process_subscription_links(text):
    ...
    global stats_subscriptions_processed, stats_subscription_configs
    ...
    for url in urls:
        ...
        if content:
            ...
            if configs:
                print(f"  ✅ Extracted {len(configs)} configs from subscription")
                all_configs.extend(configs)
                stats_subscriptions_processed += 1
                stats_subscription_configs += len(configs)
            else:
                print(f"  ⚠️ No configs found in subscription")
...
async def scrape_new_configs(client, groups, last_ids):
    ...
    for idx, group in enumerate(shuffled_groups, 1):
        ...
        for message in messages:
            ...
            # NEW: Process subscription links
    ...
    print(f"  Subscriptions processed: {stats_subscriptions_processed}")
